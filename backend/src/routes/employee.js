    ...
    try {
        ...
        if (currentUser.role === 'HR') {
            ...
            if (hrProfile && hrProfile.department) {
                whereClause = { profile: { department: hrProfile.department } };
            }
        }
        ...
        const employees = users.map(u => {
            ...
            return {
                ...
                salary: {
                    base: baseSalary,
                    allowances: u.profile?.allowances !== null && u.profile?.allowances !== undefined ? u.profile.allowances : (baseSalary * 0.1),
                    deductions: u.profile?.deductions !== null && u.profile?.deductions !== undefined ? u.profile.deductions : 200
                }
            };
        });
...
employeeRouter.put('/:empId', async (req, res) => {
    try {
        ...
        if (currentUser.role === 'HR') {
            if (targetUser.role === 'HR' || targetUser.role === 'ADMIN') {
                return res.status(403).json({ success: false, message: 'HR cannot edit themselves or other administrative accounts' });
            }
            const hrProfile = await prisma.employeeProfile.findUnique({ where: { userId: currentUser.id } });
            if (hrProfile && hrProfile.department && targetUser.profile?.department !== hrProfile.department) {
                return res.status(403).json({ success: false, message: 'HR can only edit employees in their own department' });
            }
        }
