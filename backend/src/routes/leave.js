...
leaveRouter.put('/:leaveId/status', async (req, res) => {
    try {
        ...
        if (currentUser.role === 'HR') {
            const hrProfile = await prisma.employeeProfile.findUnique({ where: { userId: currentUser.id } });
            if (hrProfile && hrProfile.department && leaveRequest.user?.profile?.department !== hrProfile.department) {
                return res.status(403).json({ success: false, message: 'HR can only update leave requests for employees in their own department' });
            }
        }
