    ...
    try {
        ...
        if (currentUser.role === 'HR') {
            ...
            if (hrProfile && hrProfile.department && targetUser.profile?.department !== hrProfile.department) {
                return res.status(403).json({ success: false, message: 'HR can only update payroll for employees in their own department' });
            }
        }

        // Update the baseSalary, allowances, and deductions in their EmployeeProfile
        await prisma.employeeProfile.upsert({
            where: { userId: targetUser.id },
            create: {
                userId: targetUser.id,
                baseSalary: Number(base),
                allowances: allowances !== undefined ? Number(allowances) : 0,
