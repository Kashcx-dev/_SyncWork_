...
export default function Calendar() {
	...
	const userLeaves = leaves.filter(
		(l) => l.empId === currentUser.empId && l.status === "Approved",
	);
	const userAttendance = attendance.filter(
		(a) => a.empId === currentUser.empId,
	);
	...
	for (let day = 1; day <= totalDays; day++) {
		...
		if (isLeave) {
			marker = {
				label: "Leave",
				style: "bg-neutral-50 text-neutral-500 border border-neutral-200 dark:bg-neutral-950 dark:text-neutral-400 dark:border-neutral-850",
			};
		} else if (dayAtt) {
			if (dayAtt.status === "Present") {
				marker = {
					label: "Present",
					style: "bg-neutral-100 text-neutral-800 border border-neutral-300 dark:bg-neutral-900 dark:text-white dark:border-neutral-750",
				};
			} else if (dayAtt.status === "Absent") {
				marker = {
					label: "Absent",
					style: "bg-neutral-950 text-white border border-neutral-850 dark:bg-neutral-800 dark:text-neutral-200 dark:border-neutral-650",
				};
			} else if (dayAtt.status === "Half-day") {
				marker = {
					label: "Half-day",
					style: "bg-neutral-200 text-neutral-700 border border-neutral-400 dark:bg-neutral-900 dark:text-neutral-300 dark:border-neutral-700",
				};
			}
		}
		...
		<div className="bg-white border border-slate-200 dark:bg-neutral-900 dark:border-neutral-800 rounded-2xl p-6 shadow-sm">
			...
			<div className="grid grid-cols-7 gap-2 text-center">
				{daysOfWeek.map((d) => (
					<div
						key={d}
						className="text-xs font-bold text-slate-400 dark:text-neutral-500 py-2 uppercase"
					>
						{d}
					</div>
				))}
				...
				{cells.map((cell, idx) => {
					if (cell.type === "empty") {
						return (
							<div
								key={cell.key}
								className="aspect-[1.2] opacity-30 bg-slate-50 dark:bg-neutral-950/20 border border-slate-100 dark:border-neutral-950 rounded-xl"
							/>
						);
					}
					return (
						...
						>
							...
							{cell.marker && (
								<span
									className={`text-[9px] font-bold px-1.5 py-0.5 rounded w-full text-center truncate ${cell.marker.style}`}
								>
									{cell.marker.label}
								</span>
							)}
