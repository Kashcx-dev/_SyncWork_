...
export const AppProvider = ({ children }) => {
    ...
    useEffect(() => {
        const handleLeaveUpdate = () => {
            ...
            if (token) {
                fetchLeaves(token);
                fetchProfileAndDashboard(token);
            }
        };
        ...
        socket.on("employee_update", handleEmployeeUpdate);

        return () => {
            socket.off("leave_update", handleLeaveUpdate);
            socket.off("employee_update", handleEmployeeUpdate);
        };
    }, []);
    ...
    const fetchAttendance = async (tokenValue) => {
        try {
            const tokenToUse = tokenValue || sessionStorage.getItem("hrms_react_token");
            if (!tokenToUse) return;
    ...
    const fetchProfileAndDashboard = async (tokenValue) => {
        try {
            ...
            if (profileRes.ok && profileData.success) {
                ...
                await fetchAttendance(tokenToUse);
            }
    ...
    const updateProfile = async (phone, address, avatar) => {
        try {
            const token = sessionStorage.getItem("hrms_react_token");
            if (!token) return;
            ...
            const res = await fetch("/api/profile", {
                ...
                body: JSON.stringify({ phone, address, avatar })
            });
            const data = await res.json();

            if (res.ok && data.success) {
                // Update local context active user details
                setCurrentUser(prev => {
                    const next = { ...prev, phone, address, avatar: avatar || prev.avatar };
                    sessionStorage.setItem("hrms_react_active_user", JSON.stringify(next));
                    return next;
                });
                await fetchProfileAndDashboard(token);
            }
        } catch (error) {
