...
export default function Chat() {
	...
	const searchUsers = async () => {
		try {
			const token = sessionStorage.getItem("hrms_react_token");
			const res = await fetch(
				`/api/chat/users?search=${encodeURIComponent(searchTerm)}`,
				{
					headers: { Authorization: `Bearer ${token}` },
				},
			);
					...
					{searchTerm.trim() !== "" ? (
						...
						<div>
							...
							{conversations.length === 0 ? (
								<div className="py-8 px-6 text-center flex flex-col items-center justify-center space-y-3 border-b border-slate-100 dark:border-neutral-800/60">
									...
									<p className="text-xs text-slate-400 dark:text-neutral-500 leading-relaxed max-w-[200px]">
										No active secure chats. Click a suggested contact below to start.
									</p>
								</div>
							) : (
								<div className="border-b border-slate-100 dark:border-neutral-800/60">
							...
							{suggestedUsers.length > 0 && (
								<div className="py-4">
									<p className="px-4 pb-2 text-[10px] uppercase font-bold text-slate-400 dark:text-neutral-500 tracking-wider">
										Suggested Contacts
									</p>
									{suggestedUsers
										.filter(user => !conversations.some(c => c.otherUser?.id === user.id))
										.map((user) => (
											<button
												key={user.id}
												onClick={() => startDirectChat(user)}
												className="w-full flex items-center gap-3 px-4 py-3 hover:bg-slate-50 dark:hover:bg-neutral-950 transition-colors text-left"
											>
												<img
													src={
														user.profile?.profilePictureUrl ||
