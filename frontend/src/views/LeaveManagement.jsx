			...
			<div className="flex justify-between items-center">
				<div>
					...
					<p className="text-xs text-slate-400 dark:text-neutral-500 mt-1">
						Submit time-off requests and track monthly calendar markers
					</p>
				</div>
				<button
					...
					<table className="w-full text-left border-collapse">
						<thead>
							<tr className="border-b border-slate-100 dark:border-neutral-800 bg-slate-50 dark:bg-neutral-950/50">
								<th className="p-4 text-xs font-bold text-slate-400 dark:text-neutral-500 uppercase">
									Date Range
								</th>
								<th className="p-4 text-xs font-bold text-slate-400 dark:text-neutral-500 uppercase">
									Leave Type
						...
						<tbody>
							{[...userLeaves].reverse().map((leave) => {
								let badgeClass =
									"bg-amber-50 text-stone-800 border border-amber-100 dark:bg-neutral-850 dark:text-neutral-300 dark:border-neutral-700";
								if (leave.status === "Rejected")
									badgeClass =
										"bg-rose-50 text-rose-700 border border-rose-100 dark:bg-neutral-950 dark:text-neutral-500 dark:border-neutral-800";
								if (leave.status === "Approved")
									badgeClass =
										"bg-emerald-50 text-emerald-700 border border-emerald-100 dark:bg-neutral-900 dark:text-white dark:border-neutral-700";

								return (
									<tr
										key={leave.id}
										className="border-b border-slate-100 dark:border-neutral-800/40 hover:bg-slate-50/50 dark:hover:bg-neutral-800/20"
									>
										<td className="p-4 text-sm font-semibold text-slate-800 dark:text-neutral-200">
										...
										<td className="p-4">
											<span
												className={`inline-block px-2.5 py-0.5 text-xs  font-bold uppercase rounded-full ${badgeClass}`}
											>
												{leave.status}
											</span>
										</td>
